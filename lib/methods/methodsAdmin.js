...
Meteor.methods({
  ...
  adminClearAllLogs: async function() {
    await Helpers.requireAdmin(this.userId);

    const logsCount = await AdminLogs.countDocuments();

    // Delete all admin logs
    await AdminLogs.removeAsync({});

    // Log this action (create a new log after clearing)
    await AdminLogs.insertAsync({
      action: 'clear_all_logs',
      adminId: this.userId,
      details: `Cleared ${logsCount} admin logs`,
      timestamp: new Date()
    });

    return { success: true, deletedCount: logsCount };
  }
